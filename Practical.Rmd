...
## GGRAPH - EXERCISES
**EXERCISE 1** Create a ggraph. Use the "news_media" data, color edges in gray (color="gray"), add labels (geom_node_text()) and represent nodes as points (geom_node_point())
```{r}
ggraph(...) + 
  geom_edge_link(...) + geom_node_...(aes(label = media), repel=TRUE) + geom_node_...()
```

**EXERCISE 2** Create a new ggraph. Use the "news_media" data, change the color depending on type of links (edges), add labels.
...
**EXERCISE 3** Do you find this graph informative? If not, why? How would you improve it?
```{r}
